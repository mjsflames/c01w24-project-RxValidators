...
router.use("*", getService("verification-service"), upload.single("file"), async (req, res, next) => {
	...
	if (req.file) {
		...
		formContent.append("file", new Blob([Buffer.from(req.file.buffer)]), req.file.originalname);
		const response = await axios.post(req.service + req.originalUrl, formContent, {
			headers: req.headers,
		});
